...
1. **開始**: 作業単位を作る。**出力された unitId を必ず控える**
   ...
   ```powershell
   uapp-dash begin --label "issue #12 の実装" --tasks "調査;実装;テスト" --claims "Assets/Scripts/**"
   ```
...
- **編集する前に `--claims` を宣言する**。シーン・プレハブ・アセット（マージ困難な YAML）は
  指定に関わらずファイル丸ごと排他として扱われ、他の作業単位と重なれば警告が出る

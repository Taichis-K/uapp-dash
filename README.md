...
> **入れる前に**: `pip install` が作る launcher exe は未署名のため、**ウイルス対策に
> 隔離・ブロックされることがある**（「インストールは成功したのにコマンドが無い」形で出る。
> 解析型では一時的にマシンが重くなる）。インストール先を除外に登録するか、除外済みの場所に
> venv を作るか、**exe を作らず `python -m uapp_dash` で使う**かを先に決めておくと事故がない。
> 手順は [SETUP.md](SETUP.md) の 1-0。`uapp-dash doctor` は exe の在るべき場所と
> そこに存在するかまで表示するので、隔離されたのか未インストールなのかを切り分けられる。

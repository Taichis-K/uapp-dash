...
- `result`（`state=done` のとき）: `success` / `failure` / `aborted` / `dropped`
  ...
  - `supersededBy`（任意・0.1.6 で追加）: `failure` / `aborted` の単位が「目的を引き継いだ
    ...
    **書き込み経路は 3 つ**（0.1.7 で②、0.1.8 で③を追加）: ①`end --superseded-by`
    （閉じるとき同時に）②後続の `begin --supersedes <旧unitId>`（後続を作るときに）
    ③`amend --unit-id <旧> --superseded-by <後続>`（両方が既に存在するときに。
    ②と違い**後続の実在を要求**する）。②③は旧単位が `failure` / `aborted` / `dropped` で
    終了済み・**引き継ぎ先が未設定**のときだけ許可。`dropped` にも付けられるのは、
